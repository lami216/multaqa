...
export const acceptJoinRequest = async (req, res) => {
  try {
    ...
    const isStudyPartnerIndividualPost =
      post.category === 'study_partner' && maxAcceptedForRole(resolvePostRole(post)) === 1;

    if (isStudyPartnerIndividualPost) {
      ...
      if (!autoGeneratedMessageExists) {
        ...
        await Message.collection.insertOne({
          ...
          text: 'مرحباً 👋 وافقت على طلبك، نقدر ننسق التفاصيل هنا',
          autoGenerated: true,
          deliveredAt: null,
          readAt: null,
          createdAt: now,
