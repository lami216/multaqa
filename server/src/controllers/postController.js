...
export const acceptJoinRequest = async (req, res) => {
  ...
  try {
    ...
    await dbSession.withTransaction(async () => {
      ...
      const joinRequest = await JoinRequest.findOne({ _id: requestId, postId: id }).session(dbSession);
      if (!joinRequest) {
      ...
      const lifecycleSession = await Session.create([{
        ...
        postSnapshot: {
          title: post.title ?? '',
          subjectCodes: post.subjectCodes ?? [],
          subjectNames,
    ...
    if (error?.code === 11000) return res.status(400).json({ error: 'Session already created for this conversation.' });
